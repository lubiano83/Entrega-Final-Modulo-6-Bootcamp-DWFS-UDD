...
export default class LodgesControllers {
    ...
    createLodge = async(req, res) => {
        try {
            ...
            if(!hotel || !size || !bedroom || !bathroom || !capacity || !high || !medium || !low) return res.status(404).send({ message: "Todos los campos son requeridos.." });
    ...
    updateLogdeById = async(req, res) => {
        try {
            const { id } = req.params;
            const lodge = await lodgesDao.getLodgesById(id);
            if(!lodge) return res.status(400).send({ message: "Ese lodge no existe.." });
            const { hotel, size, bedroom, bathroom, capacity, wifi, season } = req.body;
            const { high, medium, low } = season;
            if(!hotel || !size || !bedroom || !bathroom || !capacity || !wifi || !high || !medium || !low) return res.status(404).send({ message: "Todos los campos son requeridos.." });
