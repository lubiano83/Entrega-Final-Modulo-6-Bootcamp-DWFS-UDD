...
export default class UsersControllers {
    ...
    changeImageById = async (req, res) => {
        try {
            ...
            const stream = file.createWriteStream({ metadata: { contentType: req.file.mimetype || "image/webp" }});
            stream.on("error", (err) => {
                console.error("Error al subir la imagen:", err);
                return res.status(500).send({
                    message: "Error al subir imagen a Firebase.",
                    error: err.message,
                });
            });
            stream.on("finish", async () => {
                try {
                    ...
                    return res.status(500).send({
                        message: "Imagen subida pero no se pudo actualizar el usuario.",
                        error: error.message,
                    });
                }
            });
